<div align="center">

# Oi, eu sou o Wagner Santos 👋

**Fundador & engenheiro · Construindo o [EasyPDV](https://www.easypdv.com.br) · Brasil 🇧🇷**

*Eu construo mercados autônomos — do totem à nuvem, e tudo o que existe no meio.*

[![LinkedIn][badge-linkedin]](https://www.linkedin.com/in/waguii)
[![E-mail][badge-email]](mailto:wagner@uniolab.com.br)
[![X][badge-x]](https://x.com/waguii)

<sub>🇺🇸 Prefer English? <a href="./README.md">Read this page in English</a></sub>

</div>

---

## 👨‍💻 Sobre mim

- 🏗️ Fundador do **[EasyPDV](https://www.easypdv.com.br)** — uma plataforma completa para mercados autônomos, construída full-stack: do PDV Kotlin rodando no totem até a infraestrutura self-hosted por trás
- 🧊 **Este perfil é só a ponta do iceberg**: 4.500+ contribuições no último ano, ~99% em repositórios privados de produto
- 📚 Me aprofundando em **Machine Learning & Data Science na Le Wagon** · também engenharia com a **Vupty**
- 🤖 Regra pessoal: *se eu tiver que fazer duas vezes, eu escrevo um bot*
- 🐙 Codando no GitHub desde **2013**

## 🏪 O que estou construindo

### [EasyPDV](https://www.easypdv.com.br) — mercados autônomos, de ponta a ponta

Lojas de autoatendimento para condomínios e empresas: totem de checkout inteligente, ERP na nuvem, estoque, vendas e faturamento em tempo real, pagamentos por PIX · cartão · NFC — sem precisar de funcionários. Em produção, vendido como SaaS.

| Camada | O que está no ar |
|-------:|:-----------------|
| 📱 **Mobile** | 4 apps Android em Kotlin — PDV, autoatendimento touch (terminais Sunmi), apps do operador e do mercado |
| ⚙️ **Backend** | Java · Spring — APIs modulares & microsserviços (gateway, auth, service discovery) |
| 🖥️ **Web** | Painéis admin em TypeScript · sites em Astro · docs para clientes e devs |
| 🐍 **Dados & bots** | Crawlers em Python — enriquecimento de catálogo GTIN, automação de conteúdo e preços |
| 🚀 **Infra** | CI/CD & registry self-hosted — Docker · Harbor · Dokploy |
| 🔌 **Hardware** | Terminais de pagamento Sunmi · etiquetas eletrônicas BLE · integração com controle de acesso |

### 🧺 Despensinha

Linha de despensas inteligentes do mesmo ecossistema — micro-mercados para espaços menores: etiquetas IoT, microsserviços Java, storefronts TypeScript.

### 🔬 [Uniolab](https://github.com/Uniolab)

Meu estúdio e marca guarda-chuva — onde moram os produtos, as automações e os experimentos.

## 🛠️ Caixa de ferramentas

<div align="center">

| | |
|------------------:|:--|
| **Mobile** | <img src="https://skillicons.dev/icons?i=kotlin,androidstudio,gradle" height="42" alt="Kotlin, Android Studio, Gradle" /> |
| **Backend** | <img src="https://skillicons.dev/icons?i=java,spring,mysql" height="42" alt="Java, Spring, MySQL" /> |
| **Web** | <img src="https://skillicons.dev/icons?i=ts,angular,astro,nodejs" height="42" alt="TypeScript, Angular, Astro, Node.js" /> |
| **Data & ML** | <img src="https://skillicons.dev/icons?i=py,sklearn,opencv" height="42" alt="Python, scikit-learn, OpenCV" /> |
| **Infra & Ops** | <img src="https://skillicons.dev/icons?i=docker,linux,bash,git,githubactions" height="42" alt="Docker, Linux, Bash, Git, GitHub Actions" /> |

<sub>Também no dia a dia: Proxmox · Harbor · Dokploy · ELK — self-hosted por princípio.</sub>

</div>

## 📊 Stats — a fatia visível

> Aqui todo commit conta — o calendário abaixo inclui os **repositórios privados de produto**, onde ~99% do trabalho acontece. 🧊

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/waguii/waguii/metrics/metrics-dark.svg">
    <img alt="Métricas do GitHub do Wagner com calendário de contribuições do ano" src="https://raw.githubusercontent.com/waguii/waguii/metrics/metrics.svg">
  </picture>
  <br>
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://streak-stats.demolab.com?user=waguii&theme=tokyonight&hide_border=true&background=00000000&locale=pt_BR">
    <img height="170" alt="Sequência de contribuições do Wagner" src="https://streak-stats.demolab.com?user=waguii&hide_border=true&background=00000000&locale=pt_BR">
  </picture>
</div>

## 🎯 Agora & missões paralelas

- 🧠 **Le Wagon — Data Science & IA**: notebooks, modelos e desafios se acumulando (em privado) em 2026
- 🎮 **Ciência de draft no Dota 2** — experimentos de ML com picks de heróis: [dota2-counter-pick-helper](https://github.com/waguii/dota2-counter-pick-helper) · [octarine-pick-helper](https://github.com/waguii/octarine-pick-helper)
- 🕹️ Fuçando o código-fonte da **Unreal Engine**
- 🏠 **Homelab** — cluster Proxmox, tudo self-hosted, e o resto automatizado

## 📫 Fale comigo

**Tem — ou quer ter — um mercado autônomo?** Vamos conversar: **[easypdv.com.br](https://www.easypdv.com.br)** 🏪

Para todo o resto: [wagner@uniolab.com.br](mailto:wagner@uniolab.com.br) · [LinkedIn](https://www.linkedin.com/in/waguii) · [X @waguii](https://x.com/waguii)

---

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/waguii/waguii/output/github-contribution-grid-snake-dark.svg">
  <img alt="Animação da cobrinha no gráfico de contribuições" src="https://raw.githubusercontent.com/waguii/waguii/output/github-contribution-grid-snake.svg">
</picture>

<sub>🇺🇸 <a href="./README.md">English version</a> · feito com ☕ no Brasil</sub>

</div>

[badge-linkedin]: https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=data:image/svg%2Bxml;base64,%3D
[badge-email]: https://img.shields.io/badge/wagner%40uniolab.com.br-EA4335?style=for-the-badge&logo=gmail&logoColor=white
[badge-x]: https://img.shields.io/badge/%40waguii-000000?style=for-the-badge&logo=x&logoColor=white
